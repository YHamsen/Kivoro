...
const OKXWalletSection: React.FC<OKXWalletSectionProps> = ({ balances, tickers, onRefresh }) => {
  ...
  const walletColor = WALLET_COLORS.okx // #108EE9 - Bleu OKX
  ...
  const loadRealTimeData = async () => {
    try {
      const popularPairs = await okxApi.getPopularPairs()
      setRealTimeData(popularPairs)
    } catch (error) {
      console.error('Erreur chargement données temps réel OKX:', error)
    }
  }
  ...
  const loadOrderHistory = async () => {
    try {
      setLoading(true)
      const history = await okxApi.getOrderHistory()
      setOrderHistory(history)
    } catch (error) {
      console.error('Erreur chargement historique OKX:', error)
  ...
  const generateDepositAddress = async (coin: string) => {
    try {
      setLoading(true)
      const addressData = await okxApi.getDepositAddress(coin)
      setDepositAddress(addressData[0]?.addr || '')
  ...
  const handleQuickTrade = async (symbol: string, side: 'buy' | 'sell') => {
    try {
      const result = await okxApi.placeOrder(symbol, 'cash', side, 'market', '0.001')
      toast.success(`Ordre ${side} placé sur OKX: ${result.ordId}`)
      onRefresh?.()
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      {/* En-tête OKX avec design spécifique */}
      <div 
      ...
      >
        {/* Effet de lueur d'arrière-plan bleu OKX */}
        <div 
          className="absolute inset-0 opacity-10"
          style={{ 
            background: `radial-gradient(circle at 70% 30%, ${walletColor}, transparent 70%)`
          }}
        />
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h2 className="text-2xl font-bold text-white">OKX Wallet</h2>
                <p className="text-slate-400">Exchange Global • Liquidité Élevée</p>
          ...
          {/* Navigation interne avec couleurs OKX */}
      ...
      >
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-900 rounded-xl p-6 border border-slate-700">
              <h3 className="text-lg font-semibold text-white mb-4 flex items-center">
                ...
                Balances OKX ({balances.length} actifs)
              </h3>
                  ...
                  <div key={balance.coin} className="flex items-center justify-between p-4 bg-slate-800/50 rounded-lg hover:bg-slate-700/50 transition-colors">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <div className="text-slate-400 text-sm">OKX Trading Account</div>
                      </div>
                    </div>
                    <div className="text-right">
            ...
            <div className="bg-slate-900 rounded-xl p-6 border border-slate-700">
              ...
              <div className="space-y-3">
                {realTimeData.slice(0, 5).map((pair) => (
                  <div key={pair.instId} className="flex items-center justify-between p-4 bg-slate-800/50 rounded-lg">
                    <div className="flex items-center space-x-3">
                      <div>
                        <div className="text-white font-medium">{pair.instId}</div>
                        <div className="text-slate-400 text-sm">Vol: {parseFloat(pair.vol24h).toLocaleString()}</div>
                      </div>
                    </div>
                    <div className="flex items-center space-x-4">
                      <div className="text-right">
                        <div className="text-white font-bold">${parseFloat(pair.last).toLocaleString('fr-FR', { minimumFractionDigits: 2 })}</div>
                        <div className={`text-sm font-medium ${
                          parseFloat(pair.changePercent || '0') >= 0 ? 'text-green-400' : 'text-red-400'
                        }`}>
                          {parseFloat(pair.changePercent || '0') >= 0 ? '+' : ''}{parseFloat(pair.changePercent || '0').toFixed(2)}%
                        </div>
                      </div>
                      <div className="flex space-x-2">
                        <motion.button
                          whileHover={{ scale: 1.05 }}
                          whileTap={{ scale: 0.95 }}
                          onClick={() => handleQuickTrade(pair.instId, 'buy')}
                        ...
                        <motion.button
                          whileHover={{ scale: 1.05 }}
                          whileTap={{ scale: 0.95 }}
                          onClick={() => handleQuickTrade(pair.instId, 'sell')}
            ...
            {loading ? (
              ...
              <div className="space-y-3">
                {orderHistory.length > 0 ? orderHistory.slice(0, 10).map((order) => (
                  <div key={order.ordId} className="flex items-center justify-between p-4 bg-slate-800/50 rounded-lg">
                    <div className="flex items-center space-x-3">
                      <div 
                      ...
                      />
                      <div>
                        <div className="text-white font-medium">{order.instId}</div>
                        <div className="text-slate-400 text-sm">
                          {new Date(parseInt(order.cTime)).toLocaleDateString('fr-FR')}
                        </div>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="text-white">{order.sz} @ ${parseFloat(order.px || '0').toFixed(2)}</div>
                      <div className={`text-sm font-medium ${
                        order.state === 'filled' ? 'text-green-400' : 'text-yellow-400'
                      }`}>
                        {order.state}
                      </div>
                    </div>
        ...
        {activeTab === 'payments' && (
          <div className="space-y-6">
            {/* Génération d'adresse de dépôt OKX */}
            <div className="bg-slate-900 rounded-xl p-6 border border-slate-700">
              <h3 className="text-lg font-semibold text-white mb-4 flex items-center">
...
export default OKXWalletSection
