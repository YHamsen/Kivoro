...
import { TrendingUp, TrendingDown, BarChart3, Wallet, Send, Plus, ExternalLink, RefreshCw, AlertCircle, Check } from 'lucide-react'
...
const BinanceWalletSection: React.FC<BinanceWalletSectionProps> = ({ balances, tickers, onRefresh }) => {
  const [activeTab, setActiveTab] = useState<'wallet' | 'trading' | 'history' | 'payments'>('wallet')
  ...
  const walletColor = WALLET_COLORS.binance // #F3BA2F - Jaune Binance
  ...
  const loadRealTimeData = async () => {
    try {
      const popularPairs = await binanceApi.getPopularPairs()
      setRealTimeData(popularPairs)
    } catch (error) {
      console.error('Erreur chargement données temps réel Binance:', error)
    }
  }
  ...
  const loadOrderHistory = async () => {
    try {
      setLoading(true)
      // Récupérer les ordres pour les paires populaires
      const popularSymbols = ['BTCUSDT', 'ETHUSDT', 'SOLUSDT']
      let allOrders: any[] = []
      
      for (const symbol of popularSymbols) {
        try {
          const orders = await binanceApi.getOrderHistory(symbol, 5)
          allOrders = [...allOrders, ...orders]
        } catch (error) {
          console.error(`Erreur récupération ordres ${symbol}:`, error)
        }
      }
      
      setOrderHistory(allOrders)
    } catch (error) {
      console.error('Erreur chargement historique Binance:', error)
  ...
  const generateDepositAddress = async (coin: string) => {
    try {
      setLoading(true)
      const addressData = await binanceApi.getDepositAddress(coin)
  ...
  const handleQuickTrade = async (symbol: string, side: 'buy' | 'sell') => {
    try {
      const result = await binanceApi.placeOrder(symbol, side.toUpperCase() as 'BUY' | 'SELL', 'MARKET', '0.001')
      toast.success(`Ordre ${side} placé sur Binance: ${result.orderId}`)
      onRefresh?.()
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      {/* En-tête Binance avec design spécifique */}
      <div 
      ...
      >
        {/* Effet de lueur d'arrière-plan jaune Binance */}
        <div 
          className="absolute inset-0 opacity-10"
          style={{ 
            background: `radial-gradient(circle at 20% 80%, ${walletColor}, transparent 70%)`
          }}
        />
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h2 className="text-2xl font-bold text-white">Binance Wallet</h2>
                <p className="text-slate-400">Leader Mondial • Volume #1</p>
          ...
          {/* Navigation interne avec couleurs Binance */}
          <div className="grid grid-cols-4 gap-2 bg-slate-800/50 rounded-xl p-2 backdrop-blur-sm">
            {[
              ...
              return (
                <motion.button
                  ...
                  className={`p-3 rounded-lg transition-all duration-200 relative ${
                    isActive 
                      ? 'text-slate-900 shadow-lg font-bold' 
                ...
                >
                  ...
                  {isActive && (
                    <motion.div
                      layoutId="binance-tab-indicator"
      ...
      >
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-900 rounded-xl p-6 border border-slate-700">
              <h3 className="text-lg font-semibold text-white mb-4 flex items-center">
                ...
                Balances Binance ({balances.length} actifs)
              </h3>
                ...
                {balances.map((balance) => (
                  <div key={balance.coin} className="flex items-center justify-between p-4 bg-slate-800/50 rounded-lg hover:bg-slate-700/50 transition-colors">
                    <div className="flex items-center space-x-3">
                      <div 
                        className="w-12 h-12 rounded-full flex items-center justify-center text-slate-900 font-bold border-2"
                        style={{ 
                      ...
                      <div>
                        ...
                        <div className="text-slate-400 text-sm">Binance Spot Wallet</div>
                      </div>
                    </div>
                    <div className="text-right">
        ...
        {activeTab === 'trading' && (
          <div className="space-y-6">
            {/* Trading en temps réel Binance */}
            <div className="bg-slate-900 rounded-xl p-6 border border-slate-700">
              <h3 className="text-lg font-semibold text-white mb-4 flex items-center">
                <BarChart3 className="w-5 h-5 mr-2" style={{ color: walletColor }} />
                Trading Binance - Paires Populaires
                  ...
                  <div key={pair.symbol} className="flex items-center justify-between p-4 bg-slate-800/50 rounded-lg">
                    <div className="flex items-center space-x-3">
                      <div>
                        ...
                        <div className="text-slate-400 text-sm">Vol: {parseFloat(pair.volume).toLocaleString()}</div>
                      </div>
                    </div>
                    <div className="flex items-center space-x-4">
                      <div className="text-right">
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <div className="text-slate-400 text-sm">
                          {new Date(order.time).toLocaleDateString('fr-FR')}
                        </div>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="text-white">{order.origQty} @ ${parseFloat(order.price).toFixed(2)}</div>
                      <div className={`text-sm font-medium ${
                        order.status === 'FILLED' ? 'text-green-400' : 'text-yellow-400'
                      }`}>
                        {order.status}
                      </div>
                    </div>
        ...
        {activeTab === 'payments' && (
          <div className="space-y-6">
            {/* Génération d'adresse de dépôt Binance */}
            <div className="bg-slate-900 rounded-xl p-6 border border-slate-700">
              <h3 className="text-lg font-semibold text-white mb-4 flex items-center">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-800 border border-slate-600 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-yellow-500"
                  >
                ...
                <motion.button
                  ...
                  disabled={loading}
                  className="w-full py-3 rounded-lg font-bold transition-colors disabled:opacity-50 text-slate-900"
                  style={{ 
                    backgroundColor: walletColor
                  }}
                >
                  ...
                  <div className="p-4 bg-slate-800/50 rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <motion.button
                        ...
                        className="text-xs px-2 py-1 rounded text-slate-900 font-medium"
                        style={{ 
                          backgroundColor: `${walletColor}80`
                        }}
                      >
                        Copier
                      </motion.button>
...
export default BinanceWalletSection
